...
<?= $this->section('content') ?>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            ...
            <p class="text-outline font-medium text-lg">Sesuaikan bobot prioritas untuk setiap aspek perkembangan anak.
            </p>
        </div>
    ...
    <form action="<?= base_url('criteria/update') ?>" method="POST">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <?php foreach ($criterias as $crit): ?>
                <div
                    class="bg-surface-container-lowest rounded-xl p-6 shadow-[0_8px_24px_rgba(224,64,160,0.08)] border-2 border-primary-fixed hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden group">
                    <div
                        class="absolute top-0 right-0 w-32 h-32 bg-primary-fixed rounded-bl-full -z-10 opacity-50 group-hover:scale-110 transition-transform">
                    </div>
                    ...
                    <div class="flex justify-between items-start mb-6">
                        <div class="flex items-center gap-3">
                            <div>
                                ...
                                <span
                                    class="bg-primary-fixed text-on-primary-fixed-variant text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider"><?= $crit->type ?></span>
                            </div>
                        </div>
                    ...
                    <div class="mb-8">
                        <input name="weights[<?= $crit->id ?>]"
                            ...
                            max="100" min="0" type="range" value="<?= (float) $crit->weight ?>"
                            data-id="<?= $crit->id ?>" />
                    </div>
                        ...
                        <div class="space-y-3">
                            <?php if (!empty($crit->subs)): ?>
                                <?php foreach ($crit->subs as $sub): ?>
                                    <div
                                        class="flex justify-between items-center bg-surface-container-lowest p-3 rounded-lg border border-outline-variant shadow-sm hover:border-primary-fixed transition-colors">
                                        <span class="font-medium text-on-surface-variant"><?= $sub->sub_name ?></span>
                                        <span
                                            class="bg-secondary-container text-secondary font-bold px-3 py-1 rounded-full text-[10px]">SAW</span>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
            ...
            <button type="button" onclick="document.getElementById('modalAdd').classList.remove('hidden')"
                ...
                <div
                    class="bg-surface-container-lowest p-4 rounded-full shadow-sm mb-4 group-hover:scale-110 group-hover:bg-primary group-hover:text-on-primary transition-all duration-300">
                ...
                <p class="text-outline font-medium mt-2 text-center max-w-xs">Tambahkan aspek penilaian lainnya seperti
                    Sosial-Emosional.</p>
            </button>
        </div>
        ...
        <!-- Save Action Bar -->
        <div
            class="mt-10 bg-surface-container-lowest p-6 rounded-xl shadow-[0_8px_30px_rgba(224,64,160,0.15)] border border-primary-fixed flex flex-col sm:flex-row justify-between items-center gap-4 sticky bottom-6 z-10 backdrop-blur-sm bg-white/90">
            <div class="flex items-center gap-3">
                <div class="bg-tertiary-container text-on-tertiary-container p-2 rounded-full">
                    <span class="material-symbols-outlined">info</span>
                </div>
                <p class="font-medium text-on-surface">Pastikan total bobot kriteria mencapai 100% sebelum menyimpan.
                </p>
            </div>
            <button type="submit"
